...
function loadExam() {
    fetch("/graphql", {
        ...
        populateQuestions(result.data.questions);
...
function submitExam() {
    ...
    for (let i = 0; i < numQuestions; i++) {
        ...
        if (fields[0].type === "radio") {
            const checked = fields.find(r => r.checked);
    ...
    fetch("/graphql", {
        ...
        body: JSON.stringify({
            query: `
            mutation GradeTest($answers: [String]!) {
        ...
        if (result.data.grade.flag) {
            message = `Congratulations, your score was high enough to get a flag! ${result.data.grade.flag}`;
        }
        finalResult(`You answered ${result.data.grade.score} out of ${numQuestions} questions correctly. ${message}`, !!result.data.grade.flag)
    }).catch(e => {
        console.error(e);
        alert("An error occurred. Please contact CS Camp Staff.");
    })
